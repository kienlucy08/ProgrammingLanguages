...
-- consumes foldable collection (t) of Either ErrorT Value values
-- produces an Either ErrorT Value result
-- If successful and produces a valid value, it will be wrapped in Right.
-- If an error occurs during the computation, it will be wrapped in Left, containing the corresponding ErrorT error type.
mulValList :: Foldable t => t (Either ErrorT Value) -> Either ErrorT Value
mulValList values = mulValues values
  where
    mulValues :: Foldable t => t (Either ErrorT Value) -> Either ErrorT Value
    mulValues = foldr mul (Right (IntVal 1))

    mul :: Either ErrorT Value -> Either ErrorT Value -> Either ErrorT Value
    mul (Right (IntVal x)) (Right (IntVal acc)) = Right (IntVal (x * acc))
    mul _ _ = Left (EvalError "Invalid multiplication")
...
-- If an error occurs during the computation, it will be wrapped in Left, containing the corresponding ErrorT error type.
divValList :: Foldable t => t (Either ErrorT Value) -> Either ErrorT Value
divValList values = divValues values
  where
    divValues :: Foldable t => t (Either ErrorT Value) -> Either ErrorT Value
    divValues = foldr divOp (Right (IntVal 1))

    divOp :: Either ErrorT Value -> Either ErrorT Value -> Either ErrorT Value
    divOp (Right (IntVal x)) (Right (IntVal acc))
      | x /= 0 = Right (IntVal (acc `div` x))
      | otherwise = Left (EvalError "Division by zero")
    divOp _ _ = Left (EvalError "Invalid division")
...
-- If an error occurs during the computation, it will be wrapped in Left, containing the corresponding ErrorT error type.
modValList :: Foldable t => t (Either ErrorT Value) -> Either ErrorT Value
modValList values = modValues values
  where
    modValues :: Foldable t => t (Either ErrorT Value) -> Either ErrorT Value
    modValues = foldr modOp (Right (IntVal 0))

    modOp :: Either ErrorT Value -> Either ErrorT Value -> Either ErrorT Value
    modOp (Right (IntVal x)) (Right (IntVal acc))
      | x /= 0 = Right (IntVal (acc `mod` x))
      | otherwise = Left (EvalError "Modulo by zero")
    modOp _ _ = Left (EvalError "Invalid modulo")
...
evalNotExpr = do
  ...
  case eval evalExpr (env, expr) of
    Right (val, _) ->
      case val of
        BoolVal bool -> return (BoolVal (not bool))
        _ -> failEval "not <boolexpr> must evaluate to a bool type"
    Left err -> failEval "not <boolexpr> must evaluate to a bool type"
